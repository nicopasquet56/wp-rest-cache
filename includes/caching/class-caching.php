...
class Caching {
	...
	public function get_cache( $cache_key ) {
		...
		$should_serve_until_regenerated = $this->should_serve_until_regenerated() && filter_has_var( INPUT_GET, 'regenerate_cache' ) === false;
		if ( 1 === strtotime( $expiration ) && !$should_serve_until_regenerated ) {
			return false;
		}
	...
	public function delete_cache( $cache_key, $force = false ) {
		...
		// Leadoff: optionnally dont clear cache entry to allow serving old value until cache entry is refreshed
		if( ! $this->should_serve_until_regenerated() ) {
	...
	/**
	 * Should flushed cache entry be served until regenerated by the cron
	 *
	 * @return bool Whether or not a cron should be activated.
	 */
	public function should_serve_until_regenerated() {
		return get_option( 'wp_rest_cache_serve_flushed_cache', false ) === '1';
	}
		...
		foreach ( $results as &$result ) {
			if ( 1 === strtotime( $result['expiration'] ) || false === get_transient( $this->transient_key( $result['cache_key'] ) ) ) {
				...
				if ( $this->lock_cache_regeneration( $result['cache_key'] )) {
					...
					$url    = $home_url . $result['request_uri'] . (parse_url($result['request_uri'], PHP_URL_QUERY) ? '&' : '?') . 'regenerate_cache=1';
					$return = wp_remote_get(
						$url,
						[
							'timeout'   => get_option('wp_rest_cache_cron_timeout',10),
							'sslverify' => false,
